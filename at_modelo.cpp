...
void ATModelo::Ejecucion(){
    ...
    for (int columna=1; columna<4; columna++){
        ValYizq=Yini-(offsetY*(columna-1));
        ValXizq=Xini+(offsetX*(columna-1));

        ValYder=YiniD+(offsetY*(columna-1));
        ValXder=XiniD+(offsetX*(columna-1));

        for(int fila=0; fila<columna; fila++){

            robotEjecucion->setPunto(MOV_TRAYECTORIA_U,ValXizq,ValYizq,958,400,50,50,700);
            //qDebug()<<"punto a la izquierda";
            robotEjecucion->salidasActuadorPLC(0,true);
            if(getStopReceived()){
                break;
            }
            robotEjecucion->setPunto(MOV_TRAYECTORIA_U,ValXder,ValYder,955,400,50,50,700);
            //qDebug()<<"punto a la derecha";
            robotEjecucion->salidasActuadorPLC(0,false);
            if(getStopReceived()){
                break;
            }
            ValXizq=ValXizq-paso;
            ValXder=ValXder-paso;
        }
